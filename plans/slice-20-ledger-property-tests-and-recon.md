...
- Job is invokable as `pnpm --filter @studio/billing exec tsx scripts/reconcile.ts`
...
```bash
pnpm --filter @studio/billing add -D fast-check
```
...
```bash
pnpm test:int
DATABASE_URL=$DATABASE_URL pnpm --filter @studio/billing exec tsx scripts/reconcile.ts
```
