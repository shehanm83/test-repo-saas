...
import { z } from "zod";
import { adminInsertStock, adminListStock, deleteStock, createDb } from "@studio/db";
...
```ts
import { z } from "zod";
import { createDb, adminListPricebook, adminInsertPricebookEntry, priceBookLookup, expirePricebookVersion } from "@studio/db";
...
```bash
pnpm --filter @studio/db exec tsx scripts/seed-pricebook.ts
```
